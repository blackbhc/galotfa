...
/**
 * @class monitor
 * @brief The main server of the on-the-fly analysis, which organize the other
...
{
public:
    monitor(const std::string_view&
                tomlParaFile);  // initialize with the toml file name
    ~monitor();
    void main_analysis_api(double        time,
...
private:
    ...
    using compDataContainer = struct compDataStruct
    {
        ...
        std::unique_ptr<double[]> potentials =
            nullptr;  // potentials of particles
        std::unique_ptr<double[]> coordinates =
            nullptr;  // coordinates of particles
        std::unique_ptr<double[]> velocities =
            nullptr;  // velocities of particles
    };
    ...
    using compResContainer = struct compResStruct
    {
        ...
        std::unique_ptr<double[]> A2Re =
            nullptr;  // real parts of the radial A2 profile
        std::unique_ptr<double[]> A2Im =
            nullptr;  // imaginary parts of the radial A2 profile
    };

    // extract the data used for orbital log
    auto id_data_process(double        time,
                         unsigned      particleNumber,
        ...
        -> std::vector<monitor::orbitPoint>;
    // NOTE: API of orbital log
    void        orbital_log(double        time,
                            unsigned      particleNumber,
                            const int*    ids,
                            const int*    partTypes,
                            const double* masses,
    ...
    static auto component_data_extract(unsigned      particleNumber,
                                       const int*    partTypes,
                                       const double* masses,
                                       const double* potentials,
                                       const double* coordinates,
