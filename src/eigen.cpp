 ...
 * @paramout eigenValues the eigenvalues of the given array, sorted by their
 * absolute value.
 * @param eigenVectors 1D array of the 3x3 eigenmatrix of the corresponding
 * eigenvectors, in row-major order and each column is an eigenvector.
 * @return
 */
void eigen::eigens_sym_33(double matrixData[matrixDim],
                          double eigenValues[vecDim],
                          double eigenVectors[matrixDim])
{
    gsl_matrix_view matrixView =
        gsl_matrix_view_array(matrixData, vecDim, vecDim);
