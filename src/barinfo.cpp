 ...
 * @return the A2 value
 */
auto bar_info::A2(const unsigned partNum,
                  const double*  masses,
                  const double*  phis) -> double
{
    ...
    MPI_Allreduce(MPI_IN_PLACE, &A2sumRe, 1, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &A2sumIm, 1, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
 ...
 */
auto bar_info::bar_angle(const unsigned partNum,
                         const double*  masses,
                         const double*  phis) -> double
{
    double A2sumRe = 0;  // real part
 ...
 */
auto bar_info::Sbuckle(const unsigned partNum,
                       const double*  masses,
                       const double*  phis,
                       const double*  zeds) -> double
{
    ...
    MPI_Allreduce(MPI_IN_PLACE, &numeratorRe, 1, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &numeratorIm, 1, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
    return sqrt(numeratorRe * numeratorRe + numeratorIm * numeratorIm)
           / A0value;
}
