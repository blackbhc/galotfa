...
void print_orbital_part(otf::runtime_para& para)
{
    ...
    INFO("(%g, %g, %g)", para.orbit->recenter.initialGuess[0],
         para.orbit->recenter.initialGuess[1],
         para.orbit->recenter.initialGuess[2]);
...
void print_component_part(otf::runtime_para& para)
{
    ...
    for (auto& comp : para.comps)
    {
        ...
        {
            INFO("Alignment of [%s] is enabled.",
                 comp.second->compName.c_str());
        ...
        {
            INFO("Radial A2 profile of [%s] is enabled.",
                 comp.second->compName.c_str());
...
/**
 * @brief The main analysis API, which should be called in the main loop of the
 * simulation.
 *
 ...
 */
void monitor::main_analysis_api(const double   time,
                                ...
                                const double*  velocities)
{
    ...
    for (auto& comp : para.comps)
    {
        component_analysis(time, particleNumber, partTypes, masses, potentials,
                           coordinates, velocities, comp.second);
    }
 ...
 * @param velocity velocities of particles
 */
void monitor::orbital_log(const double   time,
                          const unsigned particleNumber,
                          const int*     ids,
                          const int*     partTypes,
                          const double*  masses,
                          ...
                          const double*  velocities)
{
    ...
    auto orbitData = id_data_process(time, particleNumber, ids, partTypes,
    ...
    if (isRootRank and stepCounter == 0)
    {
        for (auto& data : orbitData)
        {
            ...
            h5Organizer->create_dataset_in_group(
                datasetName, "Orbit", {orbitPointDim}, H5T_NATIVE_DOUBLE);
    ...
    if (isRootRank)
    {
        ...
        {
#ifdef DEBUG
            auto returnCode = h5Organizer->flush_single_block(
                "Orbit", orbitDatasetNames[i], orbitData[i].data);
            if (returnCode != 0)
            {
                ERROR("The dataset [Orbit/%s] written faild!",
                      orbitDatasetNames[i].c_str());
                throw;
            }
#else
            h5Organizer->flush_single_block(
                "Orbit", orbitDatasetNames[i].c_str(), orbitData[i].data);
#endif
        }
    }
}
 ...
 * @return an object of otf::monitor::compDataContainer, which is the container
 * of the extracted data
 */
auto monitor::component_data_extract(unsigned                    particleNumber,
                                     const int*                  partType,
                                     const double*               masses,
                                     const double*               potentials,
                                     const double*               coordinates,
    ...
    -> monitor::compDataContainer
{
    ...
    for (unsigned i = 0; i < particleNumber; ++i)
    {
        ...
        auto find_res =
            find(comp->types.begin(), comp->types.end(), partType[i]);
 ...
 *
 * @param dataContainer reference to the extracted data, in the form of
 * compDataContainer
                                  ...
                                  compResContainer&                res)
{
    ...
    */
    ...
    // get the system center based on the inital guess: 100 times enclosed
    // radius
    auto center = recenter::get_center(
        comp->recenter.method, dataContainer.partNum,
        dataContainer.masses.get(), dataContainer.potentials.get(),
        dataContainer.coordinates.get(), comp->recenter.radius * 100,
        comp->recenter.initialGuess);
    // get the system center based on the previous result: 50 times enclosed
    // radius
    center = recenter::get_center(comp->recenter.method, dataContainer.partNum,
                                  dataContainer.masses.get(),
                                  dataContainer.potentials.get(),
                                  dataContainer.coordinates.get(),
                                  comp->recenter.radius * 50, center.get());
    // get the system center based on the previous result: 10 times enclosed
    // radius
    center = recenter::get_center(comp->recenter.method, dataContainer.partNum,
                                  dataContainer.masses.get(),
                                  dataContainer.potentials.get(),
                                  dataContainer.coordinates.get(),
                                  comp->recenter.radius * 10, center.get());
    // get the system center based on the previous result: 1 times enclosed
    // radius
    center = recenter::get_center(
        comp->recenter.method, dataContainer.partNum,
        dataContainer.masses.get(), dataContainer.potentials.get(),
        dataContainer.coordinates.get(), comp->recenter.radius, center.get());
    // get the system center based on the previous result: 0.5 times enclosed
    // radius
    center = recenter::get_center(comp->recenter.method, dataContainer.partNum,
                                  dataContainer.masses.get(),
                                  dataContainer.potentials.get(),
                                  dataContainer.coordinates.get(),
                                  comp->recenter.radius * 0.5, center.get());
                                     ...
                                     std::unique_ptr<otf::component>& comp)
{
    ...
    for (unsigned i = 0; i < dataContainer.partNum; ++i)
    {
        ...
        radius = sqrt(dataContainer.coordinates[i * 3 + 0]
    ...
    //
    // # new y axis: the normal vector for the intersection line between the new
    // Oxy plane and old # x=0 plane tmp = -newZ[1] / newZ[2]  # the z-comp of
    // the new y axis newY = np.array([0, 1, tmp])  # new y axis newY = newY /
    // np.linalg.norm(newY)  # normalization
    //
    ...
    // the new y axis as the normal vector for the intersection line between the
    // new Oxy plane and old x=0 plane
    rotation[3 * 0 + 1] = 0;
    rotation[3 * 1 + 1] = 1;
...
/**
 * @brief The API to align the coordinates to inertia tensor in a data container
 * object.
 *
 * @param dataContainer reference to the data container
 * @param comp wrapper of parameters for analysis of a single component
                                   ...
                                   std::unique_ptr<otf::component>& comp)
{
    ...
    MPI_Allreduce(MPI_IN_PLACE, inertiaTensor, 9, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
...
/**
 * @brief API to calculate the bar information, namely bar strength, bar angle,
 * buckling strength.
 *
                       ...
                       compResContainer&                res)
{
    ...
    if (comp->barAngle.enable)
    {
        // extracted data
        unsigned                   count = 0;
        unique_ptr<double[]> const usedMasses(
            new double[dataContainer.partNum]);
        ...
        for (unsigned i = 0; i < dataContainer.partNum; ++i)
        {
            ...
            radius               = sqrt(dataContainer.coordinates[3 * i + 0]
                                            * dataContainer.coordinates[3 * i + 0]
                                        + dataContainer.coordinates[3 * i + 1]
                                              * dataContainer.coordinates[3 * i + 1]);
        ...
        res.barAngle =
            bar_info::bar_angle(count, usedMasses.get(), usedPhis.get());
    };
    ...
    if (comp->sBuckle.enable)
    {
        ...
        res.sBuckle = bar_info::Sbuckle(count, usedMasses.get(), usedPhis.get(),
                                        usedZeds.get());
    }
}
                         ...
                         compResContainer&                res) const
{
    ...
    for (unsigned i = 0; i < dataContainer.partNum; ++i)
    {
        ...
        usedPhis[count]   = atan2(dataContainer.coordinates[3 * i + 1],
                                  dataContainer.coordinates[3 * i + 0]);
        usedMasses[count] = dataContainer.masses[i];
        locs[count] = unsigned((radius - lowerBound) / (upperBound - lowerBound)
                               * ( double )binNum);
        ++count;
    }
    ...
    // MPI reduce
    MPI_Reduce(A2ReSend.get(), A2ReRecv.get(), binNum, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(A2ImSend.get(), A2ImRecv.get(), binNum, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(A0Send.get(), A0Recv.get(), binNum, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
                    ...
                    compResContainer&                res) const
{
    ...
    auto imageXY = statistic::bin2d(
        mpiRank, xs.get(), -comp->image.halfLength, comp->image.halfLength,
        comp->image.binNum, ys.get(), -comp->image.halfLength,
        comp->image.halfLength, comp->image.binNum, statistic_method::COUNT,
        dataContainer.partNum, dataContainer.masses.get());
    auto imageXZ = statistic::bin2d(
        mpiRank, xs.get(), -comp->image.halfLength, comp->image.halfLength,
        comp->image.binNum, zs.get(), -comp->image.halfLength,
        comp->image.halfLength, comp->image.binNum, statistic_method::COUNT,
        dataContainer.partNum, dataContainer.masses.get());
    auto imageYZ = statistic::bin2d(
        mpiRank, ys.get(), -comp->image.halfLength, comp->image.halfLength,
        comp->image.binNum, zs.get(), -comp->image.halfLength,
        comp->image.halfLength, comp->image.binNum, statistic_method::COUNT,
        dataContainer.partNum, dataContainer.masses.get());
 ...
 * @param comp otf::component object, a structure of parameters for a component
 */
void monitor::component_analysis(double                      time,
                                 ...
                                 unique_ptr<otf::component>& comp) const
{
    ...
    auto compDataContainer =
        component_data_extract(particleNumber, partTypes, masses, potentials,
    ...
    if (isRootRank and stepCounter == 0)
    {
        ...
        if (comp->sBar.enable)
        {
            h5Organizer->create_dataset_in_group("A2", comp->compName, {1},
                                                 H5T_NATIVE_DOUBLE);
        }
        if (comp->barAngle.enable)
        {
            h5Organizer->create_dataset_in_group("BarAngle", comp->compName,
                                                 {1}, H5T_NATIVE_DOUBLE);
        }
        if (comp->sBuckle.enable)
        {
        ...
        if (comp->image.enable)
        {
            ...
            h5Organizer->create_dataset_in_group(
                "ImageXY", comp->compName,
                {comp->image.binNum, comp->image.binNum}, H5T_NATIVE_DOUBLE);
            h5Organizer->create_dataset_in_group(
                "ImageXZ", comp->compName,
                {comp->image.binNum, comp->image.binNum}, H5T_NATIVE_DOUBLE);
            h5Organizer->create_dataset_in_group(
                "ImageYZ", comp->compName,
                {comp->image.binNum, comp->image.binNum}, H5T_NATIVE_DOUBLE);
        }
        ...
        if (comp->A2profile.enable)
        {
            ...
            h5Organizer->create_dataset_in_group("A2profile_Rs", comp->compName,
                                                 {comp->A2profile.binNum},
                                                 H5T_NATIVE_DOUBLE);
            double rBinSize = (comp->A2profile.rmax - comp->A2profile.rmin)
                              / comp->A2profile.binNum;
            ...
            h5Organizer->flush_single_block(comp->compName, "A2profile_Rs",
                                            A2Rs.get());
    ...
    if (isRootRank)
    {
        ...
        if (comp->recenter.enable)
        {
            h5Organizer->flush_single_block(comp->compName, "Center",
                                            compResContainer.center);
        }

        // bar infos
        if (comp->sBar.enable)
        {
            h5Organizer->flush_single_block(comp->compName, "A2",
                                            &compResContainer.sBar);
        }
        if (comp->barAngle.enable)
        {
...
/**
 * @brief Extract the orbital data points and sort them based on the particle
 * IDs, only the root rank will return the effective data.
 *
 * @param time time of the simulation
 * @param particleNumber number of particles in the local mpi rank
 * @param particleID ids of particles
    ...
    -> vector<orbitPoint>
{
    ...
    const unique_ptr<int[]> numInEachRank3D(
        new int[mpiSize]());  // number in each rank
    ...
    // gather ids
    MPI_Gatherv(getData->id.data(), localNum, MPI_INT, gIDs.get(),
                numInEachRank.get(), offsets.get(), MPI_INT, 0, MPI_COMM_WORLD);
    // gather coordinates
    MPI_Gatherv(getData->coordinate.data(), localNum * 3, MPI_DOUBLE,
                gCoordinate.get(), numInEachRank3D.get(), offsets3D.get(),
                MPI_DOUBLE, 0, MPI_COMM_WORLD);
    // gather velocities
    MPI_Gatherv(getData->velocity.data(), localNum * 3, MPI_DOUBLE,
                gVelocity.get(), numInEachRank3D.get(), offsets3D.get(),
                MPI_DOUBLE, 0, MPI_COMM_WORLD);
